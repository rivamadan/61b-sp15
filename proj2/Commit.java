...
public class Commit implements Serializable {
	...
	public void updateFileAndCommits(String addFile) {
		...
		HashMap<String, Integer> updatedFiles = new HashMap<String, Integer>(
				inheritedFiles);
		updatedFiles.put(addFile, currNode.commitNum);
		currNode.commitedFiles = updatedFiles;
	}
	...
	public TreeMap<Integer, String> getRebaseCommits(String branch) {
		...
		Node splitNode = findSplitPointNode(branch);
		String splitNodeBranch = splitNode.branch;
		while (splitNode.left != null || splitNode.right != null) {
			if (splitNode.equals(getHeadNode())) {
				return new TreeMap<Integer, String>();
			}
			if (splitNode.left != null
					&& (splitNode.left.branch).equals(splitNodeBranch)) {
				splitNode = splitNode.left;
				commits.put(splitNode.commitNum, splitNode.message);
			}
			if (splitNode.right != null
					&& (splitNode.right.branch).equals(splitNodeBranch)) {
	...
	public void changeBranchPointer(int id, String branch) {
		updateBranchPointers(branch, commitIDs.get(id));
	}
